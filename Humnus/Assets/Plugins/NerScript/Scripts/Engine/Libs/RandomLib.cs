...
namespace NerScript
{
    public static class RandomLib
    {
        ...
        public static Vector2 PointInDonuts(float radius, float dinutsRadius, float startDeg = 0.0f, float endDeg = 359.99999f)
        {
            float deg = Range(startDeg, endDeg);
            float rad = deg * Deg2Rad;
            Vector2 position = new Vector2(Cos(rad), Sin(rad));

            position *= Range(dinutsRadius, radius);
            return position;
        }
        ...
        public static T RandomItemInList<T>(List<T> list)
        {
