...
namespace NerScript
{
    ...
    public static class ListLib
    {
        ...
        /// <summary>
        /// Index前までカット
        /// {0,1,2,3,4,5,6} CutBefore(4);
        /// =>{4,5,6}
        /// </summary>
        public static List<T> CutBefore<T>(this List<T> list, int index, Action<T> removeWithAct = null)
        {
            index.ClampMax(list.Count);
            for(int i = 0; i < index; i++)
            {
                removeWithAct?.Invoke(list[0]);
                list.RemoveAt(0);
            }
            return list;
        }
        ...
        /// </summary>
        /// 5の場合、01234になる
        public static List<T> Fill<T>(this List<T> list, int count, Func<T> create = null)
        {
            while(list.Count < count)
            {
                list.Add(create == null ? default : create());
            }
            return list;
        }
        ...
        public static List<T> RemoveRangeL<T>(this List<T> list, int start, int end)
        {
            for(int i = start; i <= end; i++)
            {
                list.RemoveAt(start);
                if(list.Count <= start) return list;
            }
            return list;
        }
        public static List<T> RemoveRangeL<T>(this List<T> list, int count)
        {
            return list.RemoveRangeL(0, count - 1);
        }
        /// <summary>
        /// 要素の最後を削除
        /// </summary>
        ...
        public static T FindMin<T, R>(this List<T> self, Func<T, R> selector)
        {
            if(self.Any()) return default;

            R min = self.Min(selector);
            return self.First(c => selector(c).Equals(min));
        }

        public static T FindMax<T, R>(this List<T> self, Func<T, R> selector)
        {
            if(self.Any()) return default;

            R max = self.Max(selector);
            return self.First(c => selector(c).Equals(max));
        }

        public static int FindIndexOfMin<T, R>(this List<T> self, Func<T, R> selector)
        {
            if(self.Count == 0) return -1;

            R min = self.Min(selector);
            return self.FindIndex(c => selector(c).Equals(min));
        }

        public static int FindIndexOfMax<T, R>(this List<T> self, Func<T, R> selector)
        {
            if(self.Any()) return -1;

            R max = self.Max(selector);
            return self.FindIndex(c => selector(c).Equals(max));
        }
        ...
        /// <param name="list"></param>
        /// <returns></returns>
        public static bool NullorEnpty<T>(this List<T> list)
        {
        ...
        public static T[] ShiftArray<T>(this T obj, params T[] objs) =>
        (new List<T> { obj }).Added(objs.ToList()).ToArray();
        public static List<T> ShiftList<T>(this T obj) => new List<T> { obj };
        public static List<T> ShiftList<T>(this T obj, params T[] objs) =>
        (new List<T> { obj }).Added(objs.ToList());

        #endregion

        #region Create

        public static List<int> CreateIntList(int start, int end, int interval = 1)
        {
