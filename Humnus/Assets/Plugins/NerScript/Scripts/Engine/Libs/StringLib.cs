...
namespace NerScript
{
    public static class StringLib
    {
        ...
        /// <param name="depth"></param>
        /// <returns></returns>
        public static string GetNextDirectory(this string path, int depth = 1)
        {
            return path.Delimit(deleteSlash: false).RemoveRangeL(depth).Joined();
        }
        ...
        /// ファイル名
        /// </summary>
        public static string GetFileName(this string filePath, bool containsExtension = false)
        {
            string fileName = filePath.Split('/').Last();
            if (!containsExtension) fileName = fileName.DeleteExtension();
            return fileName;
        }
        ...
        public static string Cuted(this string str, int length)
        {
            if (length < str.Length)
                str = str.Substring(0, length);
            return str;
        }
        public static string Filled(this string str, int length, char fillChar = ' ')
        {
            if (str.Length < length)
                str = str.PadRight(fillChar);
            return str;
        }
        public static string SetedLength(this string str, int length, char fillChar = ' ')
        {
            str = str.Cuted(length);
            str = str.Filled(length, fillChar);
            return str;
        }
        public static string OmittedText(this string str, int maxLength, string opittedSymbol = "...")
        {
            if (maxLength < str.Length)
            {
                str = str.Cuted(maxLength);
                str += opittedSymbol;
            }
            return str;
        }
        ...
        public static bool Search(this string str, string filter)
        {
        ...
        public static bool ContainsAll(this string self, params string[] strs)
        {
            foreach (var str in strs)
            {
                if (!self.Contains(str)) return false;
            }
            return true;
        }
        public static bool ContainsAny(this string self, params string[] strs)
        {
            foreach (var str in strs)
            {
        ...
        public static bool StartsWithAny(this string self, params string[] strs)
        {
            foreach (var str in strs)
            {
                if (self.StartsWith(str)) return true;
            }
            return false;
        }
